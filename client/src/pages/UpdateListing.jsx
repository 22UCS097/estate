          ...
          if(files.length>0 && files.length +formData.imageUrls.length <7){
             ...
             setImageUploadError(false);
              ...
              Promise.all(promises).then((urls)=>{
                ...
                setImageUploadError('Image upload failed (2 mb ma xpre image');
                setUploading(false);
    ...
    const  handleSubmit=async (e)=>{
       ...
       try {
          if(formData.imageUrls.length<1) return setError('you must upload at least one image')
             if(+formData.regularPrice<+formData.discountPrice) return setError('discount should be less than the regular price');
         setLoading(true);
         setError(false);
                      ...
                      <div className="flex flex-wrap gap-6">
                          <div className="flex items-center gap-2">
                             <input type='number'  id="bedrooms" min='1' max='10' required 
                                className='bg-slate-400 p-3 border border-gray-300 rounded-lg ' onChange={handleChange}  value={formData.bedrooms}/>
                             <p>Beds</p>
                          </div>
                          <div className="flex items-center gap-2">
                             <input type='number'  id="bathrooms"  min='1' max='10' required 
                                className=' bg-slate-400  p-3 border border-gray-300 rounded-lg ' onChange={handleChange}  value={formData.bathrooms} />
                             <p>Bath</p>
                          </div>
                             <div className="flex items-center gap-2">
                             <input type='number'  id="regularPrice" min='50' max='1000000' required 
                                className=' bg-slate-400 p-3 border border-gray-300 rounded-lg ' onChange={handleChange}  value={formData.regularPrice}/>
                          ...
                          <div className="flex items-center gap-2">
                             <input type='number' id="discountPrice" min='0' max='1000000'  required 
                                className=' bg-slate-400 p-3 border border-gray-300 rounded-lg ' onChange={handleChange}  value={formData.discountPrice}/>
                              <div className="flex flex-col items-center">
